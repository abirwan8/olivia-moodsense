...
const TeaMenuAdmin = () => {
  ...
  const filteredItems = moodBevList.filter((item) => {
    return item.bevName.toLowerCase().includes(searchKeyword.toLowerCase()) || item.moodType.toLowerCase().includes(searchKeyword.toLowerCase());
  });
      ...
      <Container fluid>
        <Row style={{ marginTop: "24px" }}>
          <Col md={7} xs={12}>
            ...
            <p className="text-muted teanology-menu-update">Manage your mood data on this page.</p>
          </Col>
            ...
            <Modal.Body>
              <Form>
                <Form.Group className="mb-2" controlId="formBasicEmail">
                  ...
                  <Form.Select className="form-data" name="bev" value={formData.bev} onChange={handleInputChange}>
                ...
                <Form.Group className="mb-2" controlId="formBasicEmail">
                  ...
                  {moodOptions.map((option) => (
                    <Form.Check key={option.id} type="checkbox" id={option.id} label={option.type} name="mood" value={option.type} checked={formData.mood.includes(option.type)} onChange={handleCheckboxChange} />
                  ))}
                </Form.Group>
              </Form>
            </Modal.Body>
            <Modal.Footer>
              <Button className="pagination-button btn-light text-light" onClick={submitMoodBevData}>
                Save
              </Button>
                ...
                <tr key={val.id}>
                  ...
                  <td className="d-flex justify-content-center">
                    ...
                    <Modal show={showDelete} onHide={handleCloseDelete} backdrop="static" keyboard={false}>
                      ...
                      <Modal.Body>
                        <p>
                          Are you sure, want to delete mood <span className="fw-bold">{deleteNameBev}</span> ?
                        </p>
                      </Modal.Body>
                      <Modal.Footer>
                        <Button
