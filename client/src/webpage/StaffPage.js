...
const TeaMenuAdmin = () => {
  ...
  const filteredItems = staffList.filter((item) => {
    return item.name.toLowerCase().includes(searchKeyword.toLowerCase()) || item.email.toLowerCase().includes(searchKeyword.toLowerCase()) || item.role.toLowerCase().includes(searchKeyword.toLowerCase());
  });
